...
# ── Raw Fi (trades per day) used for Foptimal sensitivity ──
def trading_frequency(wallet_df):
    if len(wallet_df) < 2:
        return np.nan
    days = (wallet_df['block_time'].max() - wallet_df['block_time'].min()).days + 1
    return len(wallet_df) / max(days, 1)
...
# =========================
# 5. CROSS-SECTIONAL NORMALISATION
# =========================
# Only the 5 sub-components are rank-transformed.
# trading_frequency is kept as raw trades/day for the robustness script.
features = [
    'momentum_resistance',
    'timing_discipline',
    'frequency_control',
...
fomo_df[features] = fomo_df[features].fillna(0)
# trading_frequency: fill missing with 0 (wallets with < 2 trades)
fomo_df['trading_frequency'] = fomo_df['trading_frequency'].fillna(0)
